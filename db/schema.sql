...
-- ============================================================
-- SUPPORT REPORT OUTBOX (central back office only)
-- ============================================================
...
-- ============================================================
-- PROFESSIONAL DEVELOPMENT (personal teacher records)
-- ============================================================
CREATE TABLE IF NOT EXISTS professional_development_activities (
  id SERIAL PRIMARY KEY, owner_id TEXT NOT NULL, owner_name TEXT NOT NULL DEFAULT '', title TEXT NOT NULL,
  activity_type TEXT NOT NULL CHECK (activity_type IN ('อบรม','สัมมนา','ประชุม','ไปราชการ','ศึกษาดูงาน')),
  starts_at TIMESTAMPTZ NOT NULL, ends_at TIMESTAMPTZ, location TEXT NOT NULL DEFAULT '', organizer TEXT NOT NULL DEFAULT '',
  ...
  hours NUMERIC(6,2), expenses NUMERIC(12,2), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS professional_development_participants (
  ...
  participant_kind TEXT NOT NULL CHECK (participant_kind IN ('teacher','student','external')), reference_id TEXT,
  label TEXT NOT NULL, role_label TEXT NOT NULL DEFAULT '', class_name TEXT NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS professional_development_attachments (
  id SERIAL PRIMARY KEY, activity_id INTEGER NOT NULL REFERENCES professional_development_activities(id) ON DELETE CASCADE,
  file_key TEXT NOT NULL, file_name TEXT NOT NULL DEFAULT '', file_size BIGINT NOT NULL DEFAULT 0, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
