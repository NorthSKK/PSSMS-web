...
/**
 * คู่มือเริ่มต้นใช้งานสำหรับโรงเรียน — เปิดได้โดยไม่ต้องล็อกอิน
 *
 * ธุรการต้องอ่านได้ตั้งแต่ก่อนเข้าระบบครั้งแรก และเนื้อหาไม่มีข้อมูลของใครเลย
 * เสิร์ฟจากแอปของโรงเรียนเองเพื่อให้คู่มือตรงกับเวอร์ชันที่โรงเรียนนั้นใช้อยู่จริง
 * (ข้อความส่งมอบในทะเบียนลูกค้าลิงก์มาที่ path นี้ — เปลี่ยน path ต้องแก้ที่นั่นด้วย)
 */
app.get('/manual', (req, res) => {
  try {
    res.type('html').send(require('./lib/manual').manualPage());
  } catch (err) {
    // คู่มืออ่านไม่ได้ไม่ใช่เหตุให้ทั้งแอปพัง แต่ต้องบอกตรง ๆ ว่าอ่านไม่ได้
    console.error('[manual]', err.message);
    res.status(500).type('txt').send('เปิดคู่มือไม่สำเร็จ');
  }
});
