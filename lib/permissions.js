...
// บทบาทจัดการโรงเรียน: Admin และ Executive มีสิทธิ์ดำเนินงานของโรงเรียนเท่ากัน
// ใช้ predicate เดียวกันเพื่อไม่ให้แต่ละ endpoint ตีความ Executive ไม่ตรงกัน.
...
function managementOnly(user) {
  if (!isManagement(user)) throw new Error('สงวนสิทธิ์เฉพาะผู้ดูแลระบบหรือผู้บริหาร');
}

function teacherOrManagement(user) {
  const r = role(user);
  if (r !== 'ADMIN' && r !== 'EXECUTIVE' && r !== 'TEACHER') {
    throw new Error('สงวนสิทธิ์เฉพาะครู ผู้ดูแลระบบ หรือผู้บริหาร');
  }
}

// Alias สำหรับ endpoint อ่านข้อมูลทั้งโรงเรียนที่มีอยู่เดิม.
function adminOrExecutive(user) {
  managementOnly(user);
}

/**
 * บุคลากรของโรงเรียน = ครู + ผู้ดูแลระบบ + ผู้บริหาร — กันแค่ "ไม่ใช่นักเรียน"
 *
 * ใช้กับของที่เป็นงานของบุคลากรทั้งหมด เช่น ทะเบียนสารบรรณ
 */
function staffOnly(user) {
  const r = role(user);
  if (r !== 'ADMIN' && r !== 'TEACHER' && r !== 'EXECUTIVE') {
    throw new Error('สงวนสิทธิ์เฉพาะบุคลากรของโรงเรียน');
  }
}
...
async function verifyAttendanceBatchOwner(user, rowIds) {
  if (isManagement(user) || rowIds.length === 0) return;
  const teacherId = String(user?.id || '').trim().toLowerCase();
  const { rows } = await query(
...
// Management may override teacherId (e.g. assigning a subject to a specific teacher);
// teachers always use their own JWT identity.
function resolveTeacherId(user, payloadTeacherId) {
  return isManagement(user) ? (payloadTeacherId || String(user?.id || '')) : String(user?.id || '');
}
