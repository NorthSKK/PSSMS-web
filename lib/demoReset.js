...
async function start() {
  ...
  // สั่งล้างทันทีตอนบูต — ใช้ตอนแก้ db/seed-demo.js แล้วอยากเห็นผลโดยไม่ต้องรอถึงตี 3
  // ตั้งเป็น variable ชั่วคราวแล้วเอาออก ไม่งั้นทุก deploy จะล้างข้อมูลคนที่กำลังลองอยู่
  // ปลอดภัยเพราะ seed-demo.js ยังต้องผ่าน assertSafeToWipe() ซึ่งขอเครื่องหมายเดโมอยู่ดี
  if (process.env.DEMO_RESET_ON_BOOT === '1') {
    console.log('[demo-reset] DEMO_RESET_ON_BOOT=1 — ล้างข้อมูลทันที');
    running = true;
    runSeed().finally(() => { running = false; });
  }
