...
const {
  isDemoDatabase, markDemo, unmarkDemo, markBootstrapAttempted, bootstrapAttempted,
} = require('./instance');
...
async function run() {
  ...
  // เคยเริ่มแล้วแต่ไม่จบ — ข้อมูลที่ค้างอยู่เป็นของเราเอง ล้างทับได้
  const retry = await bootstrapAttempted();
  ...
  console.log(retry
    ? '[demo-bootstrap] รอบก่อนไม่จบ — ลองใหม่'
    : '[demo-bootstrap] ฐานข้อมูลว่าง — ตั้งเป็นเดโมและใส่ข้อมูลตั้งต้น');
  await markBootstrapAttempted();
  await markDemo();
  try {
    await runSeed();
