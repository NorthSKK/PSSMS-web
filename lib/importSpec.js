...
const SPECS = {
  ...
  curriculum: {
    title: 'ตัวชี้วัด/ผลการเรียนรู้',
    // คีย์ของคลังคือ **รหัสวิชา + รหัสตัวชี้วัด** ไม่ใช่คอลัมน์เดียว — ตรงกับ
    // unique index `curriculum_subject_standard_unique` ที่ ON CONFLICT พึ่งอยู่
    uniqueBy: ['subjectCode', 'standardCode'],
    columns: [
      { key: 'subjectCode',  label: 'รหัสวิชา', aliases: ['subject_code', 'subjectcode'],
        required: true, example: 'ว30201' },
      { key: 'subjectType',  label: 'ประเภท', aliases: ['subject_type', 'subjecttype'],
        oneOf: ['พื้นฐาน', 'เพิ่มเติม'], example: 'เพิ่มเติม' },
      { key: 'standardCode', label: 'รหัสตัวชี้วัด', aliases: ['standard_code', 'standardcode'],
        required: true, example: 'ว 4.1 ม.4/1',
        note: 'รหัสตามหลักสูตร หรือเลขลำดับก็ได้ · ห้ามซ้ำในวิชาเดียวกัน' },
      { key: 'description',  label: 'คำอธิบาย', aliases: ['description', 'detail'],
        required: true, example: 'สืบค้นและอธิบายการค้นหาความรู้ทางฟิสิกส์' },
      { key: 'evalType',     label: 'ประเมิน', aliases: ['eval_type', 'evaltype'],
        oneOf: ['ระหว่างทาง', 'ปลายทาง'], example: '', note: 'ว่างได้' },
    ],
  },
};
...
function prepareRows(kind, rows) {
  ...
  rows.forEach((raw, i) => {
    ...
    // คีย์ที่เป็นหลายคอลัมน์รวมกัน — ปล่อยผ่านแล้ว ON CONFLICT ฝั่ง DB จะทับกันเอง
    // แถวหลังกินแถวหน้าเงียบ ๆ ครูนับจำนวนแล้วไม่ตรงกับที่เห็นในไฟล์
    if (spec.uniqueBy) {
      const parts = spec.uniqueBy.map((k) => row[k]);
      if (parts.every((x) => x)) {
        const key = parts.join('|');
        const prev = seen.get(key);
        const labels = spec.uniqueBy.map((k) => spec.columns.find((c) => c.key === k).label);
        if (prev) errors.push({ row: excelRow, message: `${labels.join(' + ')} "${parts.join(' ')}" ซ้ำกับแถว ${prev}` });
