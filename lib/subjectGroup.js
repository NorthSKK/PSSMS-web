...
/**
 * แถวที่ **หน้าครูที่ปรึกษาประจำชั้นเป็นเจ้าของ** — โฮมรูม (`HR`) กับแนะแนว/วิถีพุทธ (`-`)
 *
 * `setAllHomeroomTeachers` สร้างและลบทั้งชุดเอง (5 วัน × ครูที่ปรึกษา + คาบ 7 อีกสอง)
 * การนำเข้าตารางสอนจึงต้องไม่แตะ — เดิม `importTimetableCSV` ล้างทั้งเทอมรวมแถวพวกนี้
 * ครูที่ปรึกษาหายทั้งโรงเรียนทุกครั้งที่อัปตารางสอน โดยไม่มีอะไรบอก
 */
function isHomeroomManagedSubject(subjectCode) {
  const code = String(subjectCode || '').trim().toUpperCase();
  return code === 'HR' || code === '-';
}

module.exports = {
  SUBJECT_GROUP_BY_PREFIX, subjectPrefixOf, subjectGroupOf,
  isHomeroomSubject, isHomeroomManagedSubject,
};
