 ...
 *
 * ⚠️ สถานะปัจจุบัน: **พักฟีเจอร์อัปโหลด PDF ไว้** เพราะ production ยังไม่มีที่เก็บถาวร
 *    filesystem ของ Railway เป็น ephemeral ไฟล์หายทุก deploy จึงปิดไว้จนกว่าจะตั้ง
 *    MEDIA_STORAGE_DIR ชี้ไป Railway Volume (ดู WEB_DEV.md) — การ์ดแบบลิงก์ใช้ได้ปกติ
 */
const fs = require('fs');
...
/**
 * อัปโหลดเปิดได้ก็ต่อเมื่อมีที่เก็บ "ถาวร" จริง คือตั้ง MEDIA_STORAGE_DIR ไว้
 *
 * ไม่ตั้ง = เขียนลง filesystem ของ container ซึ่งบน Railway หายทุก deploy
 * ปล่อยให้อัปได้ในสภาพนั้นคือหลอกครู — ไฟล์หายเงียบ ๆ โดยไม่มีใครรู้จนกว่าจะกดเปิด
 * จึงปิดฟีเจอร์ไปเลยจนกว่าจะมีที่เก็บ (Railway Volume / object storage)
 */
...
async function status() {
  if (!isConfigured()) {
    return {
      connected: false,
      dir: ROOT,
      reason: 'ยังไม่ได้ตั้ง MEDIA_STORAGE_DIR — พักฟีเจอร์อัปโหลด PDF ไว้ก่อน ' +
              'เพราะไฟล์จะหายทุกครั้งที่ deploy (ดู WEB_DEV.md)',
    };
  }
  try {
    await ensureReady();
