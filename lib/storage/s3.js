 ...
 *   ตอนที่อยากให้เร็วที่สุด และดึง blob 100MB จองการเชื่อมต่อจาก pool ที่มีแค่ 20 ตัว
 *
...
// เพดานของค่าที่ผู้เรียกขอมาได้ — กันจุดใดจุดหนึ่งขอ TTL ยาวจนลิงก์ที่หลุดใช้ได้ทั้งวัน
...
async function put({ buffer, ext }) {
  const type = types.byExt(ext);
  if (!type) throw new Error('ชนิดไฟล์ไม่ถูกต้อง');
...
/**
 * อัปแบบ stream — byte ไหลผ่าน Railway แต่ **ไม่ถูกกองไว้ใน RAM ทั้งก้อน**
 *
 * เซ็นได้เพราะ aws4fetch ตั้ง `X-Amz-Content-Sha256: UNSIGNED-PAYLOAD` ให้เองสำหรับ s3
 * จึงไม่ต้อง hash body ก่อนส่ง (ซึ่งจะบังคับให้ต้องอ่านทั้งไฟล์เข้าหน่วยความจำอยู่ดี)
 *
 * `duplex: 'half'` จำเป็นตั้งแต่ Node 18 เมื่อ body เป็น stream — ไม่ใส่แล้ว fetch โยนทันที
 * `Content-Length` ต้องมี R2 ไม่รับ chunked encoding สำหรับ PutObject
 */
async function putStream({ stream, size, ext }) {
  // source อาจเปิดไฟล์ไม่สำเร็จระหว่างรอเซ็น ต้องรับ error ตั้งแต่ก่อน await แรก
  const sourceDone = finished(stream, { cleanup: true });
  sourceDone.catch(() => {});
  try {
    const type = types.byExt(ext);
    if (!type) throw new Error('ชนิดไฟล์ไม่ถูกต้อง');
    if (!Number.isSafeInteger(size) || size <= 0) throw new Error('ขนาดไฟล์ไม่ถูกต้อง');
    const key = newKey(ext);
    ...
    // aws4fetch.fetch retry 429/5xx ด้วย stream เดิมที่อ่านไปแล้ว จึงเซ็นครั้งเดียว
    // แล้วส่งด้วย native fetch; ครู retry ไฟล์นี้จากต้นฉบับได้เมื่อได้รับข้อผิดพลาด
    const res = await fetch(signed);
    if (!res.ok) {
      throw new Error(`อัปโหลดขึ้นที่เก็บไฟล์ไม่สำเร็จ (${res.status}): ${await safeText(res)}`);
    }
    // ไม่ถือ response connection ค้างไว้ (PutObject ปกติไม่มีเนื้อหา)
 ...
 */
/**
 * `ttlSeconds` ยาวได้ถึง 1 ชม. สำหรับสื่อการสอน — ดูเหตุผลที่ disk.js
 * ไม่ส่งมา = ใช้ URL_TTL_SECONDS เหมือนเดิม (งานสารบรรณ)
 */
async function getFileUrl({ key, filename, ttlSeconds }) {
  assertKey(key);
  const type = types.byExt(types.extOf(key));
  const url = new URL(objectUrl(key));
  const ttl = Number(ttlSeconds) > 0 ? Math.min(Number(ttlSeconds), MAX_URL_TTL_SECONDS)
                                     : URL_TTL_SECONDS;
  url.searchParams.set('X-Amz-Expires', String(ttl));
  url.searchParams.set('response-content-type', type.mime);
  if (filename) {
    // docx เสิร์ฟเป็น attachment — บังคับดาวน์โหลด ไม่ให้เบราว์เซอร์พยายามเรนเดอร์เอง
    url.searchParams.set(
      'response-content-disposition',
      `${type.disposition}; filename*=UTF-8''${encodeURIComponent(filename)}`
    );
  }
...
/**
 * อ่านผ่านเว็บแอปเป็น stream — ใช้เฉพาะตัวอ่าน PDF ในหน้าเว็บ เพื่อให้ pdf.js
 * ดึงไฟล์จาก origin เดียวกันได้แม้ bucket ยังไม่ได้ตั้ง CORS. ไม่กองไฟล์ใน RAM.
 */
async function getStream(key) {
  assertKey(key);
  const res = await client().fetch(objectUrl(key), { method: 'GET' });
  if (!res.ok || !res.body) throw new Error(`อ่านไฟล์จากที่เก็บไม่สำเร็จ (${res.status})`);
  return {
...
module.exports = {
  name, isConfigured, put, putStream, remove, getFileUrl, getStream, check,
  URL_TTL_SECONDS, MAX_URL_TTL_SECONDS,
};
