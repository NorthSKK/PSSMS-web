...
/**
 * เขียนจาก stream — ใช้กับไฟล์ใหญ่ที่ไม่ควรถือทั้งก้อนไว้ใน RAM
 * ล้มกลางทางต้องลบไฟล์ที่เขียนค้างทิ้ง ไม่งั้นเหลือไฟล์ครึ่ง ๆ ที่ไม่มีแถวไหนชี้ถึง
 */
async function putStream({ stream, ext }) {
 ...
 */
/**
 * `ttlSeconds` ยาวได้ถึง 1 ชม. สำหรับสื่อการสอน — ไม่ใช่ค่าเผื่อ ๆ
 * PDF viewer ของเบราว์เซอร์โหลดไฟล์ใหญ่แบบ Range request ทยอยขอตอนเลื่อน
 * ตั๋วหมดอายุระหว่างอ่าน = PDF ค้างกลางเล่ม (โหมดอ่านในหน้าเดิมคนเปิดค้างนาน)
 * งานสารบรรณไม่ส่งค่านี้มา จึงยังได้ 10 นาทีเท่าเดิม
 */
async function getFileUrl({ kind, id, user, ttlSeconds }) {
  const ticket = jwt.sign(
    { kind: String(kind), id: Number(id), uid: String(user?.id || '') },
    process.env.JWT_SECRET,
    { expiresIn: Number(ttlSeconds) > 0 ? Number(ttlSeconds) : 600 }
  );
...
// ตัวอ่าน PDF ผ่าน Railway ใช้ stream เดียวกันกับ route /file แต่ไม่ต้องเปิด CORS
async function getStream(key) {
  const stat = await fsp.stat(safePath(key));
  return { stream: readStream(key), size: stat.size };
}

module.exports = {
  name, ROOT,
  isConfigured, put, putStream, remove, getFileUrl, check,
  ensureReady, safePath, statSync, readStream, getStream,
};
