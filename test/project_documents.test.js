...
test('เอกสารโครงการดูร่วมกันได้ แต่ครูแก้ได้เฉพาะของตัวเองและ Admin แก้ได้', async () => {
  ...
  assert.ok(row, 'ครูคนอื่นต้องเห็นเอกสารโครงการร่วมกัน');
  assert.equal(row.mine, false, 'ครูคนอื่นต้องไม่มีสิทธิ์แก้');
  await assert.rejects(() => ok('saveProjectDocument', [{ id: created.id, title: 'แก้ไม่ได้' }], 'teacher2'), /เฉพาะโครงการของคุณ/);
...
test('นักเรียนเปิดคลังเอกสารโครงการไม่ได้', async () => {
