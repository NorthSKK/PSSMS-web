...
async function attachProjectFile(id, file, user) {
  ...
  if (!Number.isInteger(projectId) || !row) throw new Error('ไม่พบโครงการนี้');
  assertOwner(user,row,'แนบไฟล์ได้เฉพาะโครงการของคุณ');
  ...
  try {
    await query('INSERT INTO project_files(project_id,file_key,file_name,file_size) VALUES($1,$2,$3,$4)',
      [projectId,saved.key,clean(file.originalname,255) || ('โครงการ.' + file.detectedExt),saved.size]);
...
async function deleteProjectDocument([id], user) {
  ...
  if (!row) throw new Error('ไม่พบโครงการนี้');
  assertOwner(user,row,'ลบได้เฉพาะโครงการของคุณ');
  ...
  return { status:'success', message:'ลบโครงการแล้ว' };
}
async function deleteProjectFile([id], user) {
  ...
  assertOwner(user,file,'ลบไฟล์ได้เฉพาะโครงการของคุณ');
