...
// 100MB ต่อไฟล์ — หนังสือทั้งเล่มที่สแกนมาพอดีอยู่ในนี้ ไม่ต้องให้ครูไปบีบอัดเอง
//
// ⚠️ เพดานจริงคือ **น้อยกว่า** ค่านี้ 1 ไบต์ — multer ตัดที่ `>=` ส่วนด่านล่างตัดที่ `>`
//    ไฟล์ 100MB เป๊ะจึงถูกปฏิเสธ (ต่างกัน 1 ไบต์ ปล่อยไว้โดยรู้ตัว)
//
// ขยับเลขนี้แล้วดูสองอย่างที่ผูกกันอยู่: `RATE_LIMIT` ใน routes/media.js
// (เป็นไบต์/ชม. — เพดานไฟล์โตแล้วจำนวนไฟล์ที่อัปได้ต่อชั่วโมงลดตาม) และ
// ค่า fallback ในหน้าครู (`Scripts_General.html` — ที่เขียน `|| 100` ไว้)
