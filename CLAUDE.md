...
| มาแล้วหนี | ม่วง `#7c3aed` | สีนอกชุดสถานะ — เป็นอาการที่ต้องรีบสุด ต้องสะดุดตา |
| โดด | เทาเข้ม `#334155` | = ปุ่ม **โดด** |
| สาย | เหลืองอำพัน `--p-warning` | = ปุ่ม **สาย** |

⚠️ **ต้องเป็น 4 เฉดคนละโทน** — เดิมขาดโรงเรียนกับมาแล้วหนีเป็นแดงสองเฉด และโดดกับสาย
เป็นน้ำตาลสองเฉด แยกไม่ออกในลิสต์ · เพิ่มอาการใหม่ต้องเลือกโทนที่ยังว่าง

⚠️ **`--sw-skip` มี override ใน `.dark-mode`** (`#64748b`) เทาเข้มจมไปกับพื้นมืด
อีกสามสีคอนทราสต์พอไม่ต้องแก้

`.mg-skip` (ตารางเช็คย้อนหลัง) ใช้ token เดียวกัน — เดิมเป็นแดงเข้มที่นั่นแต่เกือบดำบนปุ่ม
**`โดด` เคยมีสองสีในระบบเดียวกัน**
