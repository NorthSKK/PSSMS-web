...
`main` ขึ้น **เดโม** (`demo.pssms.app`) และ `production` ขึ้น **โรงเรียนจริงทุกแห่ง**
(รวม `pw.pssms.app`) เพื่อให้ตรวจเดโมก่อนแตะข้อมูลจริง

> **คำสั่งเผยแพร่:** “เอาขึ้นเว็บโรงเรียน” และ “ขึ้นเว็บจริง” หมายถึงให้ push
> `production` แล้วตรวจทุกโรงเรียนที่สถานะเปิดใช้งานใน
> `https://pssms.app/admin/customers` ไม่ใช่เพียงเว็บ `pw.pssms.app`.
...
| คลังเอกสารโรงเรียน (`POST /api/media/project/:id`) | PDF / JPEG / PNG / DOCX | 100MB ต่อไฟล์ | โควตาโรงเรียน |
...
- **อัปโหลด**: ผ่าน backend เสมอ (ทั้งสอง driver) เพื่อคงการตรวจ magic bytes
  ...
  ⚠️ **ยิงทีละไฟล์ ไม่ใช่ก้อนเดียวหลายไฟล์** — multer พักสื่อลงดิสก์ชั่วคราว
  อ่านเฉพาะหัวไฟล์เพื่อตรวจชนิด แล้ว `putStream` ไปที่เก็บ จึงไม่ถือทั้งไฟล์ใน RAM
  ลบไฟล์พักทั้งสำเร็จและผิดพลาด · S3 เซ็นครั้งเดียวแล้ว native fetch ไม่ retry stream เดิม
  สารบรรณยังใช้ `memoryStorage()` เพดาน 10MB
...
หน้า **ฝ่ายบริหารงานทั่วไป → คลังเอกสารโรงเรียน** เป็นคลังกลางของโรงเรียน แยกประเภท
